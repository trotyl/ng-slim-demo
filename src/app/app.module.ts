import { ApplicationModule, ErrorHandler, NgModule, Renderer2, RendererFactory2, RendererStyleFlags2, RendererType2, Sanitizer, SecurityContext } from '@angular/core'
...
export class DomPluginRenderer implements  Renderer2 {
  ...
  listen(target: HTMLElement, eventName: string, callback: (event: any) => (boolean | void)): () => void {
    ...
    if ((parts.length !== 0) && (domEventName === 'keydown' || domEventName === 'keyup')) {
      ...
      eventCallback = (event: KeyboardEvent) => {
        if (event.key.toLowerCase() === key) { callback(event) }
      }
    }
